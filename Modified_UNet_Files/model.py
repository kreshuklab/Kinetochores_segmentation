...
def get_multiheads(in_channels, out_channels):
    conv1 = nn.Conv3d(in_channels, 8, 3, padding=1)
    conv2 = nn.Conv3d(8, 4, 3, padding=1)
    
    conv3 = nn.Conv3d(4, out_channels, 1)
    return nn.Sequential(conv1, conv2, conv3, nn.ReLU())

    #return nn.Sequential(nn.Conv3d(in_channels, out_channels, 1), nn.ReLU())
...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        # Defining 2 heads here

        self.head1 = get_multiheads(f_maps[0], out_channels)
        self.head2 = get_multiheads(f_maps[0], out_channels)
    ...
    def forward(self, x):
        ...
        # Apply sigmoid for segmentation head

        self.final_activation = nn.Sigmoid()
        x1 = self.final_activation(x1)

        ## Original network -> returned single output
        #return x

        ## For 2 heads setup, I get 2 outputs
        return x1, x2
