...
class _AbstractDiceLoss(nn.Module):
    ...
    def forward(self, input1, input2, target1, target2, weight):
        # get probabilities from logits
        raise NotImplementedError
        # input = self.normalization(input)
        #
...
class MultiHeadLoss(_AbstractDiceLoss):
    def __init__(self, weight=None):
        super().__init__(weight)#, sigmoid_normalization=None)
    ...
    def dice(self, input, target, weight):
        ...
        return compute_per_channel_dice(g_input, g_target, weight=None)

    def regloss(self, input, target):
        SmoothL1 = SmoothL1Loss()
        return SmoothL1(input, target)

    def forward(self, input1, input2, target1, target2, weight):
        #loss1 = compute_per_channel_dice(g_input, g_target1, weight=self.weight)
        loss_seg = self.dice(input1, target1, weight=None)

        loss_reg = self.regloss(input2, target2)
...
class GaussianDiceLoss(_AbstractDiceLoss):
    """Computes Dice Loss with Gaussian addition
    """
...
def compute_reg_time_loss(input):
    all_logits = []
    for cur_idx in range(input.shape[0]):
        all_logits.append(input[cur_idx])
    ...
    all_loss = 0
    for idx in range(len(all_logits) - 1):
        all_loss += compute_reg_mse(all_logits[idx], all_logits[idx + 1])
    return all_loss

def compute_reg_mse(logits1, logits2, smooth=1):
    curr_flat = logits1.flatten()
    next_flat = logits2.flatten()

    mse = nn.MSELoss()
    return mse(curr_flat, next_flat)

class TemporalRegMSE:
    def __init__(self, **kwargs):
        pass

    def __call__(self, input, target):
        mse_loss = nn.MSELoss()
        return mse_loss(input, target) + compute_reg_time_loss(input)
...
SUPPORTED_LOSSES = ['BCEWithLogitsLoss', 'BCEDiceLoss', 'CrossEntropyLoss', 'WeightedCrossEntropyLoss',
            ...
            'TagsAngularLoss', 'MSELoss', 'SmoothL1Loss', 'L1Loss', 'WeightedSmoothL1Loss', 'TemporalDiceLoss', 'TemporalDistributionLoss']
...
def _create_loss(name, loss_config, weight, ignore_index, pos_weight):
    if name == 'BCEWithLogitsLoss':
        ...
        sigmoid_normalization = loss_config.get('sigmoid_normalization', True)
        return TemporalDiceLoss(weight=weight, sigmoid_normalization=sigmoid_normalization)
    elif name == 'TemporalDistributionLoss':
        sigmoid_normalization = loss_config.get('sigmoid_normalization', True)
        return TemporalDistributionLoss(weight=weight, sigmoid_normalization=sigmoid_normalization)
    elif name == 'MultiHeadLoss':
        return MultiHeadLoss(weight=weight)
