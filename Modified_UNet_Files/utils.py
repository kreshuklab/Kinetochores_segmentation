...
class SliceBuilder:
    ...
    def __init__(self, raw_datasets, label_datasets, weight_dataset, patch_shape, stride_shape, **kwargs):
        ...
        if label_datasets is None:
            ...
            self._label_slices_seg = None
            self._label_slices_reg = None
        else:
            ...
            self._label_slices_seg = self._build_slices(label_datasets[0], patch_shape, stride_shape)
            self._label_slices_reg = self._build_slices(label_datasets[0], patch_shape, stride_shape)
            #assert len(self._raw_slices) == len(self._label_slices)
            assert len(self._raw_slices) == len(self._label_slices_seg)
            assert len(self._raw_slices) == len(self._label_slices_reg)
    ...
    @property
    def label_slices_seg(self):
        return self._label_slices_seg

    @property
    def label_slices_reg(self):
        return self._label_slices_reg
