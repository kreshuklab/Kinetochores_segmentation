...
##############################################################
    ...
    def train(self, train_loader):
        ...
        for i, t in enumerate(train_loader):
            ...
            input, target1, target2, weight = self._split_training_batch(t)

            output1, output2, loss = self._forward_pass(input, target1, target2, weight)
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if hasattr(self.model, 'final_activation') and self.model.final_activation is not None:
                    output1 = self.model.final_activation(output1)
                    output2 = self.model.final_activation(output2)

                # compute eval criterion
                if not self.skip_train_validation:
                    eval_score = self.eval_criterion(output1, output2, target1, target2)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, t in enumerate(val_loader):
                ...
                # Not logging images currently

                # if i % 100 == 0:
                #     self._log_images(input, target1, target2, output, 'val_')
    ...
    def _split_training_batch(self, t):
        ...
        weight = None
        if len(t) == 3:
            input, target1, target2 = t
        else:
            input, target1, target2, weight = t
        return input, target1, target2, weight


    def _forward_pass(self, input, target1, target2, weight=None):
        # forward pass
        output1, output2 = self.model(input)

        loss = self.loss_criterion(output1, output2, target1, target2, weight)
