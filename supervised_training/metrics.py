...
#     """
#     Provide the number of instances based on watershed output.
#     """

#     def __init__(self, **kwargs):
#         pass

#     def check_gt(self, intersection_peaks, gt_coords):
...
#     def check_predictions(self, intersection_peaks, gt_coords):
...
#         watershed_peaks = []
#         watershed_instances = np.where(watershed_output[0]!=0)

#         # print('watershed_instances\n')
#         # print(watershed_instances)

#         for instance_id, instance in enumerate(watershed_instances):
#             watershed_peaks.append([watershed_instances[0][instance_id], watershed_instances[1][instance_id], watershed_instances[2][instance_id]])

#         # print('watershed_peaks\n')
#         # print(watershed_peaks)

#         intersection_peaks = []
#         for local_peak in local_max:
#             z, y, x = local_peak[0], local_peak[1], local_peak[2]
#             for wshed_peak in watershed_peaks:
#                 w_peak_z, w_peak_y, w_peak_x = wshed_peak[0], wshed_peak[1], wshed_peak[2]
#                 if z == w_peak_z and y == w_peak_y and x == w_peak_x:
#                     intersection_peaks.append(wshed_peak)

#         # print(intersection_peaks)
...
#         tp_count, fn_count = self.check_gt(intersection_peaks, gt_coords)
#         fp_count = self.check_predictions(intersection_peaks, gt_coords)
...
#         #print(len(np.unique(watershed_output)))
#         #return torch.tensor(len(np.unique(watershed_output)))
#         #return torch.tensor(F1_score)
#         return torch.tensor(tp_count)
