...
class _AbstractDiceLoss(nn.Module):
    ...
    def forward(self, input, target, kernel_size, sigma):
        # get probabilities from logits
        input = self.normalization(input)

        # compute per channel Dice coefficient
        per_channel_dice = self.dice(input, target, kernel_size, sigma, weight=self.weight)
...
class GaussianDiceLoss(_AbstractDiceLoss):
    def __init__(self, weight=None, sigmoid_normalization=True):
        super().__init__(weight, sigmoid_normalization)
        #self.smoothing = GaussianSmoothing(1, 5, 1, 3).to('cuda')
