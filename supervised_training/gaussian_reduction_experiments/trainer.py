...
class UNet3DTrainer:
    ...
    def train(self, train_loader):
        ...
        for i, t in enumerate(train_loader):
            ...
            if self.num_epoch < 2:
                output, loss = self._forward_pass(input, target, 7, 2, weight)
            elif self.num_epoch > 1 and self.num_epoch < 4:
                output, loss = self._forward_pass(input, target, 5, 1, weight)
            else:
                output, loss = self._forward_pass(input, target, 3, 1, weight)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, t in enumerate(val_loader):
                ...
                #output, loss = self._forward_pass(input, target, kernel, sigma, weight)
                if self.num_epoch < 2:
                    output, loss = self._forward_pass(input, target, 7, 2, weight)
                elif 1 < self.num_epoch < 4:
                    output, loss = self._forward_pass(input, target, 5, 1, weight)
                else:
                    output, loss = self._forward_pass(input, target, 3, 1, weight)
                if i % 100 == 0:
    ...
    # 	return GaussianSmoothing(1, 5, 1, 3)

    def _forward_pass(self, input, target, kernel, sigma, weight=None):
        # forward pass
        #smoothing = GaussianSmoothing(1, 3, 1, 3)
        #input = F.pad(input, (1,1,4,4,4,4), mode='replicate')
        ...
        # g_output = F.pad(output, (2,2,2,2,2,2), mode='replicate')
        # g_target = F.pad(target, (2,2,2,2,2,2), mode='replicate')

        #smoothing = self.add_gaussian()
        #print(type(output))
        #g_output = self.add_gaussian(output.cpu())
        #g_target = self.add_gaussian(target.cpu())

        #g_output = g_output.to(device)
        #g_target = g_target.to(device)

        #g_output = smoothing(output.cpu())
        #g_target = smoothing(target.cpu())

        # g_output = smoothing(g_output)
        # g_target = smoothing(target)

        # g_output.to('cuda')
        # g_target.to('cuda')

        #opshape = g_output.shape
        #tarshape = g_target.shape

        # compute the loss
        if weight is None:
            loss = self.loss_criterion(output, target, kernel, sigma)
        else:
            loss = self.loss_criterion(output, target, kernel, sigma, weight)

        return output, loss#, opshape, tarshape
